...
$late_payment_fine = 0;
$fine_newly_applied = false;

if($current_bill && $current_bill['status'] == 'Unpaid') 
{
    $current_date = date('d');
    
    if($current_date>22 && $current_bill['late_fine']==0) 
    {
        $late_payment_fine = 150;
        $update_fine_stmt = $conn->prepare("
            UPDATE bill 
            SET late_fine = ?, fine_applied_date = NOW() 
            WHERE id = ?
        ");
        $update_fine_stmt->bind_param("di", $late_payment_fine, $current_bill['id']);
        
        if($update_fine_stmt->execute()) 
        {
            $fine_newly_applied = true;
            $current_bill['late_fine'] = $late_payment_fine;
            $current_bill['fine_applied_date'] = date('Y-m-d H:i:s');
        }
        $update_fine_stmt->close();
    } 
    else 
    {
        $late_payment_fine = $current_bill['late_fine'];
    }
}
else if($current_bill && $current_bill['status'] == 'Paid')
{
    // For paid bills, just show the late_fine if it exists
    $late_payment_fine = $current_bill['late_fine'];
}
...
$all_bills_stmt = $conn->prepare("
    SELECT id, month, year, previous_reading, present_reading, 
           ...
           duepayment, late_fine, status, bill_datetime, 
           fine_applied_date, payment_date 
    FROM bill 
    WHERE scno = ? 
...
foreach($bills as $bill) 
{
    ...
    if($bill['status']=='Paid') 
    {
        $total_paid += ($bill['duepayment'] + $bill['late_fine']);
    } 
    else 
    {
        $total_due += ($bill['duepayment'] + $bill['late_fine']);
    }
}
...
<body>
    ...
    if (isset($_SESSION['payment_success'])) {
        $payment_type = $_SESSION['payment_success'];
        echo '<div class="success-message">';
        echo '✅ Payment successful! ';
        
        if ($payment_type == 'with_fine' && isset($_SESSION['last_payment_details'])) {
            $details = $_SESSION['last_payment_details'];
            echo 'Total paid: ₹' . number_format($details['amount'], 2);
            echo ' (Including late payment fine of ₹' . number_format($details['late_fine'], 2) . ')';
        } else {
            echo 'All bills marked as paid.';
        }
        
        echo '</div>';
        unset($_SESSION['payment_success']);
        unset($_SESSION['last_payment_details']);
    }
    
    if ($current_bill && $current_bill['status'] == 'Unpaid') {
        if ($fine_newly_applied) {
            echo '<div class="late-payment-notice">';
            echo '⚠️ Late Payment Fine of ₹' . number_format($late_payment_fine, 2) . ' has been added to your bill (Payment date exceeded 22nd of the month)';
            echo '</div>';
        } elseif ($late_payment_fine > 0) {
            echo '<div class="late-payment-notice">';
            echo '⚠️ This bill includes a Late Payment Fine of ₹' . number_format($late_payment_fine, 2);
            echo '</div>';
        }
    }
    ?>
    ...
    <div class="logout-container">
        <form action="main.html" method="post" style="display: inline;">
    ...
    <div class="bill-container">
        ...
        <?php if ($current_bill['status'] == 'Paid'): ?>
        <div class="paid-info-box">
            <h3>✓ BILL PAID</h3>
            <p><strong>Payment Date:</strong> <?php echo !empty($current_bill['payment_date']) ? date('d-M-Y', strtotime($current_bill['payment_date'])) : 'N/A'; ?></p>
            <p><strong>Amount Paid:</strong> ₹<?php echo number_format($current_bill['duepayment'] + $current_bill['late_fine'], 2); ?></p>
            <?php if ($current_bill['late_fine'] > 0): ?>
            <div class="fine-paid-notice">
                ⚠️ <strong>Note:</strong> This payment included a late payment fine of ₹<?php echo number_format($current_bill['late_fine'], 2); ?>
                <?php if (!empty($current_bill['fine_applied_date'])): ?>
                <br>Fine was applied on: <?php echo date('d-M-Y', strtotime($current_bill['fine_applied_date'])); ?>
                <?php endif; ?>
            </div>
            <?php endif; ?>
        </div>
        ...
        <div class="section">
            ...
            <table class="charges-table">
                ...
                <tbody>
                    ...
                    <?php if ($late_payment_fine > 0): ?>
                    <tr class="late-payment-row">
                        <td colspan="2"><strong>⚠️ Late Payment Fine<?php echo ($current_bill['status'] == 'Paid') ? ' (PAID)' : ' (After 22nd)'; ?></strong></td>
                        <td class="amount-col"><strong><?php echo number_format($late_payment_fine, 2); ?></strong></td>
                    </tr>
                    <?php endif; ?>
                    <tr class="total-row">
                        <td colspan="2"><strong>TOTAL AMOUNT</strong></td>
                        <td class="amount-col"><strong><?php echo number_format($current_bill['total_amount'] + $late_payment_fine, 2); ?></strong></td>
                    </tr>
                </tbody>
            </table>
        </div>
        
        <div class="amount-due-box <?php echo ($current_bill['status'] == 'Paid') ? 'paid' : ''; ?>">
            <div class="label"><?php echo ($current_bill['status'] == 'Paid') ? 'TOTAL AMOUNT PAID' : 'TOTAL AMOUNT PAYABLE'; ?></div>
            <div class="amount">₹ <?php echo number_format($current_bill['duepayment'] + $late_payment_fine, 2); ?></div>
            <div style="margin-top: 10px; font-size: 12px;">
                Payment Status: <strong><?php echo $current_bill['status']; ?></strong>
            </div>
            <?php if ($late_payment_fine > 0): ?>
            <div style="margin-top: 5px; font-size: 11px; color: <?php echo ($current_bill['status'] == 'Paid') ? '#c8e6c9' : '#ffeb3b'; ?>;">
                <?php echo ($current_bill['status'] == 'Paid') ? '✓' : '⚠️'; ?> <?php echo ($current_bill['status'] == 'Paid') ? 'Paid with' : 'Includes'; ?> Late Payment Fine: ₹<?php echo number_format($late_payment_fine, 2); ?>
            </div>
            <?php if (!empty($current_bill['fine_applied_date'])): ?>
            <div style="margin-top: 3px; font-size: 10px; color: <?php echo ($current_bill['status'] == 'Paid') ? '#c8e6c9' : '#ffeb3b'; ?>;">
                Fine Applied On: <?php echo date('d-M-Y', strtotime($current_bill['fine_applied_date'])); ?>
            </div>
            <?php endif; ?>
            <?php endif; ?>
            <?php if ($current_bill['status'] == 'Paid' && !empty($current_bill['payment_date'])): ?>
            <div style="margin-top: 5px; font-size: 11px; color: #c8e6c9;">
                Paid on: <?php echo date('d-M-Y', strtotime($current_bill['payment_date'])); ?>
            </div>
            <?php endif; ?>
        </div>
        ...
        <div class="section">
            ...
            <div class="payment-info">
                <div>
                    <?php if ($current_bill['status'] == 'Unpaid'): ?>
                    ...
                    <p style="margin-top: 10px; color: #d32f2f; font-weight: bold;">
                        ⚠️ Late payment fine of ₹<?php echo number_format($late_payment_fine, 2); ?> has been applied as payment date is after 22nd of the month.
                    </p>
                    <?php endif; ?>
                    <?php else: ?>
                    ...
                    <p><strong>Amount Paid:</strong> ₹<?php echo number_format($current_bill['duepayment'] + $current_bill['late_fine'], 2); ?></p>
                    <?php if ($current_bill['late_fine'] > 0): ?>
                    <p style="margin-top: 10px; color: #f57c00; font-weight: bold;">
                        ⚠️ This payment included a late payment fine of ₹<?php echo number_format($current_bill['late_fine'], 2); ?>
                    </p>
                    <?php endif; ?>
                ...
                <div class="qr-section">
                    ...
                    <p><?php echo ($current_bill['status'] == 'Paid') ? 'Payment Completed' : 'Scan to Pay'; ?></p>
                </div>
            </div>
        </div>
        ...
        <div class="bill-footer">
            ...
            <p>• Late payment fine of ₹150 will be charged if payment is made after 22nd of the month</p>
        ...
        <div style="padding: 20px; text-align: center; background: #f9f9f9; border-top: 2px solid black;">
            <form action="pay.php" method="post">
                <button type="submit" style="background: #4caf50; color: white; border: none; padding: 15px 40px; font-size: 16px; font-weight: bold; border-radius: 5px; cursor: pointer; font-family: Arial, sans-serif;">
                    PAY NOW - ₹<?php echo number_format($current_bill['duepayment'] + $late_payment_fine, 2); ?>
                </button>
            </form>
        </div>
        <?php endif; ?>
